...
// Small TVET classification badge (trade / level, and school when present).
function TvetBadge({ course }: { course: CourseListItem }) {
    const hasTrade = !!(course.trade_name || course.level);
    if (!hasTrade && !course.school_code) return null;
    return (
        <>
            {hasTrade && (
                <span className="shrink-0 ml-1.5 px-1.5 py-0.5 rounded text-[10px] font-medium bg-cyan-50 text-cyan-700 border border-cyan-200 align-middle">
                    {course.trade_name || 'Trade'}{course.level ? ` · L${course.level}` : ''}
                </span>
            )}
            {course.school_code && (
                <span className="shrink-0 ml-1 px-1.5 py-0.5 rounded text-[10px] font-medium bg-amber-50 text-amber-700 border border-amber-200 align-middle">
                    {course.school_name || course.school_code}
                </span>
            )}
        </>
    );
        ...
        <div ref={containerRef} className="relative w-full max-w-md">
            ...
            >
                <span className={selectedCourse ? 'text-gray-900' : 'text-gray-500'}>
                    ...
                    {selectedCourse && <TvetBadge course={selectedCourse} />}
                </span>
                <svg
