...
class course_report_helper {
    ...
    public static function get_courses_list(): array {
        ...
        // Course enrichment metadata (trade / level / school), keyed by course id.
        $metabycourse = [];
        foreach ($DB->get_records('elby_course_meta', null, '',
                'id, courseid, trade_code, trade_name, level, school_code, school_name') as $m) {
            $metabycourse[(int) $m->courseid] = $m;
        }
        ...
        foreach ($courses as $course) {
            ...
            $context = context_course::instance($course->id);
            $meta = $metabycourse[(int) $course->id] ?? null;
            $coursesbycategory[$catid][] = [
                'id' => $course->id,
                'shortname' => $course->shortname,
                'fullname' => $course->fullname,
                'enrolled_count' => count_enrolled_users($context),
                'trade_code' => $meta->trade_code ?? null,
