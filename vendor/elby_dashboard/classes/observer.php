...
class observer {
    ...
    /**
     * Enrich a newly created course with trade/level/school metadata.
     *
    ...
    public static function course_created(\core\event\course_created $event): void {
        try {
            course_enricher::enrich_course((int) $event->courseid, ['creator_userid' => (int) $event->userid]);
        } catch (\Exception $e) {
            debugging('course_created enrich failed: ' . $e->getMessage(), DEBUG_DEVELOPER);
        }
    }

    /**
     * Re-derive course metadata after an edit (e.g. recategorisation).
     *
    ...
    public static function course_updated(\core\event\course_updated $event): void {
        try {
            course_enricher::enrich_course((int) $event->courseid);
        } catch (\Exception $e) {
            debugging('course_updated enrich failed: ' . $e->getMessage(), DEBUG_DEVELOPER);
        }
    }

    /**
     * Queue Phase 2 fan-out when a new cohort is created.
     *
     * Only queues work; the course fan-out runs on cron via link_cohort_adhoc.
     *
     * @param \core\event\cohort_created $event The event.
     */
    public static function cohort_created(\core\event\cohort_created $event): void {
        try {
            $task = new \local_elby_dashboard\task\link_cohort_adhoc();
            $task->set_custom_data(['cohortid' => (int) $event->objectid]);
            \core\task\manager::queue_adhoc_task($task, true); // true = do not duplicate same custom data.
        } catch (\Throwable $e) {
            debugging('cohort_created queue failed: ' . $e->getMessage(), DEBUG_DEVELOPER);
        }
    }

    /**
     * Remove course metadata when a course is deleted.
     *
     * @param \core\event\course_deleted $event The event.
     */
    public static function course_deleted(\core\event\course_deleted $event): void {
        global $DB;
        $DB->delete_records('elby_course_meta', ['courseid' => (int) $event->courseid]);
    }
