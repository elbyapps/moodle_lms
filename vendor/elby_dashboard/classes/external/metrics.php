...
class metrics extends external_api {
    ...
    public static function get_student_list(string $schoolcode = '', int $courseid = 0,
        ...
        $select = "SELECT u.id AS userid, u.firstname, u.lastname,
                          su.sdms_id,
                          " . ($isteacher ? "et.position" : "COALESCE(st.program, '') AS program") . ",
        ...
        if (!$isteacher) {
            $programsql = "SELECT s.program_code,
                                  COALESCE(MAX(s.program), s.program_code) AS program_name
    ...
    public static function get_trades_report(string $levelname = ''): array {
        ...
        if (!empty($params['level_name'])) {
            $where = " AND s.study_level = :levelname";
            $sqlparams['levelname'] = $params['level_name'];
        }

        $sql = "SELECT s.program_code AS combination_code,
                       MAX(s.program) AS combination_name,
                       MAX(s.program) AS combination_desc,
                       COUNT(DISTINCT su.schoolid) AS school_count
                FROM {elby_students} s
                JOIN {elby_sdms_users} su ON su.id = s.sdms_userid AND su.user_type = 'student'
                WHERE s.program_code IS NOT NULL AND s.program_code != '' {$where}
                GROUP BY s.program_code
        ...
        foreach ($trades as $trade) {
            $schoolssql = "SELECT DISTINCT sc.school_code, sc.school_name
                           FROM {elby_students} s
                           JOIN {elby_sdms_users} su ON su.id = s.sdms_userid AND su.user_type = 'student'
                           JOIN {elby_schools} sc ON sc.id = su.schoolid
                           WHERE s.program_code = :code" . $where . "
            ...
            $levelssql = "SELECT DISTINCT s.class_grade AS grade_name
                            FROM {elby_students} s
                            JOIN {elby_sdms_users} su ON su.id = s.sdms_userid AND su.user_type = 'student'
                           WHERE s.program_code = :code" . $where . "
    ...
    public static function trigger_task_parameters(): external_function_parameters {
        return new external_function_parameters([
            'task_name' => new external_value(PARAM_TEXT,
                'Task to run: compute_user_metrics or aggregate_school_metrics'),
        ]);
    }
