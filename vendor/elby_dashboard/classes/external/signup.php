...
class signup extends external_api {
    ...
    public static function lookup_for_signup(string $sdmscode, string $usertype): array {
        ...
        try {
            ...
            if ($usertype === 'student') {
                ...
                $sdmsid = $data->sdmsStaffNumber ?? $sdmscode;
            }
            ...
            return [
                ...
                'study_level' => ($usertype === 'student') ? ($data->levelName ?? '') : '',
                'class_grade' => ($usertype === 'student') ? ($data->gradeName ?? '') : '',
                'class_group' => ($usertype === 'student') ? ($data->classGroupName ?? '') : '',
                'program' => $program,
                'program_code' => $programcode,
                'status' => $data->registrationStatus ?? ($data->employmentStatus ?? ''),
                'academic_year' => $data->currentAcademicYear ?? ($data->academicYear ?? ''),
                'position' => ($usertype === 'staff') ? ($data->positionName ?? '') : '',
            ];
        } catch (\Exception $e) {
    ...
    public static function register_sdms_user(string $sdmscode, string $usertype, string $password, string $username = ''): array {
        ...
        try {
            ...
            // Set the user_type profile field so the self-registered account is fully onboarded.
            require_once($CFG->dirroot . '/local/elby_dashboard/lib.php');
            local_elby_dashboard_set_user_type($userid, $usertype === 'student' ? 'Student' : 'Teacher');
