...
function local_elby_dashboard_myprofile_navigation(
    ...
    if ($sdmsuser) {
        ...
        // User is NOT linked. RTB Staff / External are intentionally not SDMS-linked,
        // so skip the SDMS Information section entirely for them.
        $usertype = local_elby_dashboard_get_user_type($user->id);
        if ($usertype === 'RTB Staff' || $usertype === 'External') {
            return;
        }
...
/**
 * Read the current value of the user_type custom profile field.
 *
 * @param int $userid Moodle user ID.
 * @return string The stored value (e.g. 'Student'), or '' if unset.
 */
function local_elby_dashboard_get_user_type(int $userid): string {
    global $DB;
    $sql = "SELECT d.data
              FROM {user_info_data} d
              JOIN {user_info_field} f ON f.id = d.fieldid
             WHERE f.shortname = :sn AND d.userid = :uid";
    $value = $DB->get_field_sql($sql, ['sn' => 'usertype', 'uid' => $userid]);
    return ($value !== false && $value !== null) ? (string) $value : '';
}

/**
 * Store the user_type custom profile field value.
 *
 * @param int $userid Moodle user ID.
 * @param string $value One of: Student, Teacher, RTB Staff, External.
 */
function local_elby_dashboard_set_user_type(int $userid, string $value): void {
    global $DB;
    $fieldid = $DB->get_field('user_info_field', 'id', ['shortname' => 'usertype']);
    if (!$fieldid) {
        return;
    }
    ...
    if ($existing) {
        ...
        $DB->insert_record('user_info_data',
            (object) ['userid' => $userid, 'fieldid' => $fieldid, 'data' => $value, 'dataformat' => 0]);
    }
}
 ...
 *
 * Complete = a user type is set, and for students/teachers an SDMS link exists.
 *
 * @param int $userid Moodle user ID.
 * @return bool
 */
function local_elby_dashboard_is_onboarded(int $userid): bool {
    global $DB;
    $type = local_elby_dashboard_get_user_type($userid);
    if ($type === '') {
        return false;
    }
    if ($type === 'Student' || $type === 'Teacher') {
        return $DB->record_exists('elby_sdms_users', ['userid' => $userid]);
    }
    return true;
}
 ...
 * Off by default; enable via the force_onboarding setting once tested. An
 * emergency escape hatch is available by setting
 * $CFG->local_elby_dashboard_disable_onboarding = true; in config.php.
 */
function local_elby_dashboard_after_require_login(): void {
    global $USER, $SESSION, $CFG, $PAGE;

    // Never gate non-interactive requests.
    ...
    // Emergency escape hatch + feature switch.
    if (!empty($CFG->local_elby_dashboard_disable_onboarding)) {
        return;
    }
    if (!get_config('local_elby_dashboard', 'force_onboarding')) {
        return;
    }
    if (during_initial_install() || !isloggedin() || isguestuser()) {
        return;
    }
    if (!empty($SESSION->elby_onboarded)) {
        return;
    }
    ...
    // Always-allowed pages (so users can complete onboarding and log out).
    if (strpos($path, '/login/') !== false) {
        return;
    }
    if (substr($path, -strlen('/local/elby_dashboard/onboard.php')) === '/local/elby_dashboard/onboard.php') {
        return;
    }
    ...
    $returnurl = ($PAGE && $PAGE->has_set_url()) ? $PAGE->url->out_as_local_url(false) : '/';
    redirect(new moodle_url('/local/elby_dashboard/onboard.php', ['returnurl' => $returnurl]));
}
